...
function createKnowledgeBaseFolders() {
  ...
  Logger.log(`✓ Knowledge base folder structure created: ${folderId}`);
...
function gatherCategoryKnowledge(category, config) {
  ...
  if (!config.KNOWLEDGE_BASE_FOLDER_ID) {
    Logger.log('⚠ No knowledge base folder ID configured. Creating folder structure...');
  ...
  if (!categoryFolders.hasNext()) {
    Logger.log(`⚠ Category folder "${folderName}" not found. Creating...`);
...
function gatherInspirationSources(config) {
  ...
  if (!sheet) {
    Logger.log('⚠ Inspiration_Sources sheet not found. Skipping inspiration gathering.');
    return [];
  }
...
function ensureWordPressCategories(config) {
  ...
  Logger.log('✓ WordPress categories verified');
}
...
function fetchWordPressPosts(categoryId, count, config) {
  ...
  try {
    ...
    if (response.getResponseCode() !== 200) {
      Logger.log(`⚠ Failed to fetch WordPress posts: ${response.getContentText()}`);
      return [];
    }
...
function updateSelfLearningDatabase(data, config) {
  ...
  Logger.log(`✓ Updated self-learning database (Gen #${data.generationNumber})`);
}
...
${context1.selfLearningActive ? '🧠 Self-Learning: ACTIVE - Apply learned optimizations' : '📚 Building Baseline: Focus on establishing quality foundation'}
...
✓ Compelling title (50-60 characters, keyword-optimized)
✓ URL-friendly slug (lowercase, hyphens only)
✓ Meta description (EXACTLY 155 characters - count carefully)
✓ Full HTML content with proper semantic tags (<h2>, <h3>, <p>, <ul>/<ol>)
✓ Word count within target range
✓ Confidence score ≥ ${config.CONFIDENCE_THRESHOLD}%
✓ 3-5 relevant keywords (primary + secondary)
✓ 2-3 internal links (paths like /services, /about, /contact)
✓ Detailed DALL-E 3 image prompt (200-400 chars)
✓ SEO-optimized image alt text
✓ Compelling excerpt (<150 characters)

CONTENT QUALITY STANDARDS:
✓ Use ONLY information from knowledge base files - DO NOT fabricate
✓ Professional B2B tone - authoritative but accessible
✓ Specific examples and actionable insights
✓ Clear structure with logical flow
✓ Natural keyword integration (no stuffing)
✓ Value-driven content that helps busy executives
...
✓ Match the professional, innovative tone of a premium automation company
✓ Provide genuine value - these are read by real decision-makers
✓ Use knowledge base content faithfully - cite specific examples from the files
✓ Be honest with confidence scores - used for continuous improvement
✓ ${context1.selfLearningActive ? 'Apply learned patterns while maintaining brand voice' : 'Build strong foundation for future learning'}
...
function buildKnowledgeSection(context, label) {
  ...
  if (context.knowledgeBase.fileCount === 0) {
    section += `⚠ No knowledge base files found for ${context.category}.\n`;
    ...
    context.knowledgeBase.files.forEach((file, i) => {
      ...
      if (file.isNew) section += `🆕 NEWLY ADDED - Use this fresh information!\n`;
...
TONE & VOICE:
• Professional but approachable (formality: 7/10)
• Technical depth balanced with accessibility (technical: 6/10)
• Confident and authoritative without arrogance
• Conversational elements to maintain engagement

STRUCTURE:
• Clear H2 sections (4-6 main sections)
• Supporting H3 subsections where needed
• Paragraphs: 3-5 sentences (100-150 words each)
• Mix of paragraphs and lists (70% use bullet points)
• Include 2-3 concrete examples per article
• Add relevant statistics/metrics where appropriate
...
SEO BEST PRACTICES:
• Primary keyword in title, first paragraph, and 2-3 times in body
• Natural keyword integration (1-2% density)
• 2-3 internal links to /services, /about, /contact, /blog
...
• Average confidence: ${context1.selfGenerated.patterns.avgConfidence}%
• Average word count: ${context1.selfGenerated.patterns.avgWordCount}
• Success rate: ${context1.selfGenerated.patterns.successRate}%
...
function generateArticlesWithClaude(context1, context2, config) {
  ...
  try {
    ...
    Logger.log(`✓ Claude generated 2 articles`);
...
- NO amateur stock photo clichés
...
function comprehensiveVerification(articles, config) {
  ...
  if (warnings.length > 0) {
    Logger.log('⚠ Verification warnings:');
    warnings.forEach(w => Logger.log(`  ${w}`));
  }

  Logger.log('✓ Verification passed (6 layers)');
}
...
function generateAndPublishDailyContent() {
  ...
  try {
    ...
    Logger.log(`\n${'═'.repeat(79)}`);
    Logger.log(`✅ SUCCESS - Generation #${generationNumber} complete in ${duration.toFixed(2)}s`);
    Logger.log('═'.repeat(79));
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p>${config.TODAY} • Generation #${generationNumber}</p>
      <span class="badge ${selfLearningActive ? 'badge-learning' : 'badge-building'}">
        ${selfLearningActive ? '🧠 Self-Learning Active' : '📚 Building Baseline'}
      </span>
    </div>
    ...
    <div class="article">
      ...
      ${publishResults.article1.success ?
        `<a href="${publishResults.article1.url}" class="article-link">View Article →</a>` :
        `<p style="color: #dc2626;">❌ Publishing failed: ${publishResults.article1.error}</p>`}
    </div>
    ...
    <div class="article">
      ...
      ${publishResults.article2.success ?
        `<a href="${publishResults.article2.url}" class="article-link">View Article →</a>` :
        `<p style="color: #dc2626;">❌ Publishing failed: ${publishResults.article2.error}</p>`}
    </div>
  ...
  try {
    MailApp.sendEmail({
      ...
      subject: `✅ ${successRate}% Success | ${successCount} Articles Published | Gen #${generationNumber}${selfLearningActive ? ' 🧠' : ''}`,
      htmlBody: htmlBody
    });
...
function handleError(error, executionId, startTime, config) {
  ...
  Logger.log(`\n❌ ERROR after ${duration.toFixed(2)}s:`);
  ...
  if (config && config.EMAIL_RECIPIENTS) {
    try {
      MailApp.sendEmail({
        to: config.EMAIL_RECIPIENTS,
        subject: `❌ Content Generation Failed - ${error.name}`,
...
function onOpen() {
  ...
  ui.createMenu('🚀 Content System')
    .addItem('▶️ Generate & Publish Now', 'generateAndPublishDailyContent')
    .addSeparator()
    .addItem('⏰ Setup Daily 4AM Trigger', 'setupDailyTrigger')
    .addItem('🗑️ Delete All Triggers', 'deleteTriggers')
    .addSeparator()
    .addItem('🏗️ Initialize All Sheets', 'initializeSheets')
    .addItem('📁 Create Knowledge Base Folders', 'createKnowledgeBaseFolders')
    .addItem('🏷️ Setup WordPress Categories', 'setupWordPressCategories')
    .addSeparator()
...
function showSetupGuide() {
  const html = HtmlService.createHtmlOutput(`
    ...
    <ol>
      <li>Use menu: Content System → Initialize All Sheets</li>
      <li>Use menu: Content System → Create Knowledge Base Folders</li>
      <li>Use menu: Content System → Setup WordPress Categories</li>
    ...
    <ol>
      <li>Use menu: Content System → Generate & Publish Now (test run)</li>
      <li>Check email notification and WordPress for published posts</li>
      <li>Use menu: Content System → Setup Daily 4AM Trigger</li>
    </ol>
